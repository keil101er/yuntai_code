...
//    }
    ...
    {   
       ...
       //电机控制模式切换 电机控制数据过渡
		...
		//****************************************************************************************************************************
		//CAN_gimbal_send__to_chassis(&hcan1,gimbal_control.gimbal_yaw_motor.absolute_angle);
...
//        shoot_can_set_current =  shoot_control_loop();       //射击任务控制循环
        ...
        pitch_can_set_current = gimbal_control.gimbal_pitch_motor.given_current;
            ...
            else  if(receive_chassis_data.mode_flag==1)
            {
...
static void gimbal_control_loop(gimbal_control_t *control_loop)
{
    ...
    // pitch轴控制
    if (control_loop->pitch_in_transition)//
    {
        uint32_t current_time = osKernelSysTick();
        uint32_t elapsed_time = current_time - control_loop->pitch_transition_start_time;
        if (elapsed_time < control_loop->pitch_transition_duration)
        {
