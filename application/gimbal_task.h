...
#define PITCH_SPEED_PID_KP        0.15f
#define PITCH_SPEED_PID_KI        0.0002f
#define PITCH_SPEED_PID_KD        0.0001f
...
#define YAW_GYRO_ABSOLUTE_PID_KD         0.00f//新
#define YAW_GYRO_ABSOLUTE_PID_MAX_OUT    10.2f
...
//#define PITCH_GYRO_ABSOLUTE_PID_KP 50.0f//15.0f//15
//#define PITCH_GYRO_ABSOLUTE_PID_KI 0.0f
//#define PITCH_GYRO_ABSOLUTE_PID_KD -1.0f//新步兵参数
...
//pitch gyro angle close-loop PID params, max out and max iout
//pitch 角度环 角度由陀螺仪解算 PID参数以及 PID最大输出，积分输出
#define PITCH_GYRO_ABSOLUTE_PID_KP 0.05f//15.0f//15
...
//pitch gyro angle close-loop PID params, max out and max iout
//pitch 角度环 角度由陀螺仪解算 PID参数以及 PID最大输出，积分输出
#define PITCH_AUTO_ABSOLUTE_PID_KP 60.0f//20
#define PITCH_AUTO_ABSOLUTE_PID_KI 0.5f
#define PITCH_AUTO_ABSOLUTE_PID_KD -2.0f//新步兵参数
...
//yaw gyro angle close-loop PID params, max out and max iout
//yaw 角度环 角度由陀螺仪解算 PID参数以及 PID最大输出，积分输出
#define YAW_AUTO_ABSOLUTE_PID_KP      0.0021f//15.0f//15
#define YAW_AUTO_ABSOLUTE_PID_KI       0.0f
#define YAW_AUTO_ABSOLUTE_PID_KD      0.0f//新步兵参数
#define YAW_AUTO_ABSOLUTE_PID_MAX_OUT  5.0f
#define YAW_AUTO_ABSOLUTE_PID_MAX_IOUT  0.0f


////自瞄到达临界值时的调用的pid参数，加入积分

//yaw gyro angle close-loop PID params, max out and max iout
//yaw 角度环 角度由陀螺仪解算 PID参数以及 PID最大输出，积分输出
// #define YAW_AUTO_LOW_ABSOLUTE_PID_KP       28.0f//60
#define YAW_AUTO_LOW_ABSOLUTE_PID_KP       10.0f//60
...
#define YAW_AUTO_LOW_ABSOLUTE_PID_KD       -0.2f
...
#define PITCH_ENCODE_RELATIVE_PID_KD -2.0f
...
#define YAW_RC_SEN    0.000032f
#define PITCH_RC_SEN  0.00003f //0.005
...
#define GIMBAL_CALI_MOTOR_SET_4310  400
...
typedef struct
{
	...
	gimbal_PID_t gimbal_motor_low_auto_angle_pid;
	...
	fp32 given_current_yaw;
	float last_auto_data;
} gimbal_motor_t;

typedef struct
{
    fp32 max_yaw;//校准最大yaw
    fp32 min_yaw;//校准最小yaw
    fp32 max_pitch;//校准最大pitch
    fp32 min_pitch;//校准最小pitch
...
//	  c_fbpara_t  C_data;
    gimbal_motor_t gimbal_yaw_motor;  //yaw轴电机的电机参数
    gimbal_motor_t gimbal_pitch_motor;//pitch轴电机的电机参数
	
    gimbal_step_cali_t gimbal_cali;
	
	fp32 pitch_transition_start_angle;  // pitch轴过渡起始角度
	
    fp32 pitch_transition_target_angle; // pitch轴过渡目标角度
	
    uint32_t pitch_transition_start_time; // pitch轴过渡开始时间
	
    uint32_t pitch_transition_duration;  // pitch轴过渡持续时间
	
    uint8_t pitch_in_transition;        // pitch轴是否处于过渡状态
