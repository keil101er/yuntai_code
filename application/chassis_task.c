...
#define angle_to_radian  0.01745f
#define Pi 3.1415926
double s=0;
double x=0;
...
{
  	//static  uint8_t u8SuperCapTime = 0; 
	  //wait a time 
    //空闲一段时间
...
static void chassis_init(chassis_move_t *chassis_move_init)
{
	...
	PID_init(&chassis_move_init->buffer_pid, PID_POSITION,power_pid,POWER_PID_MAX_OUT, POWER_PID_MAX_IOUT);
...
static void chassis_feedback_update(chassis_move_t *chassis_move_update)
{
	...
	fp32 angle_motion=chassis_move_update->chassis_yaw_motor->relative_angle; //获得当前云台与底盘的相对角度rad
...
void chassis_rc_to_control_vector(fp32 *vx_set, fp32 *vy_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    // 将坐标系旋转 90 度
    vx_set_channel = -vy_channel * CHASSIS_VX_RC_SEN;  // 原来的 vy 对应新的 vx
    vy_set_channel = -vx_channel * CHASSIS_VY_RC_SEN; // 原来的 -vx 对应新的 vy
...
/**
  ...
  * @retval         none
  */
float random_time=0,random_speed=0;
static void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
   ...
   else if (chassis_move_control->chassis_mode == CHASSIS_VECTOR_NO_FOLLOW_YAW)
    {
		//加入前馈的小陀螺模式
		// 获取底盘的角速度w
        fp32 new_wz = chassis_move_control->wz_set; 
        // 引入前馈
        fp32 new_angle = chassis_move_control->chassis_yaw_motor->relative_angle + k * new_wz; 
        fp32 sin_yaw = 0.0f, cos_yaw = 0.0f;
 
        sin_yaw = arm_sin_f32(-new_angle);
        cos_yaw = arm_cos_f32(-new_angle);
...
//        cos_yaw = arm_cos_f32(-chassis_move_control->chassis_yaw_motor->relative_angle);
		...
		const chassis_move_t *chassis_move_mode;
        chassis_move_control->vx_set = cos_yaw * vx_set + sin_yaw * vy_set;
        chassis_move_control->vy_set = -sin_yaw * vx_set + cos_yaw * vy_set;
        //"angle_set" is rotation speed set-point
        //“angle_set” 是旋转速度控制
//        chassis_move_control->wz_set=7.0f;//angle_set
        ...
        //变速小陀螺
            ...
            {
            srand(HAL_GetTick());
            random_speed = (rand() % 5) * 0.5 + 5.5; // 0 0.5 1  9
            random_time=0;
            }
			
			  first_order_filter_cali(&chassis_move_control->chassis_cmd_slow_set_wz, random_speed);
...
static void Omnidirectional_wheels(const fp32 vx_set, const fp32 vy_set, const fp32 wz_set, const fp32 angle_motion, fp32 wheel_speed[4])
{
    // 角度调整，坐标系旋转 90 度
    fp32 adjusted_angle = angle_motion - 90.0f * angle_to_radian; // 旋转 90 度

    // 四个轮子的运动公式
    wheel_speed[0] = -(-my_sin((45 + adjusted_angle) * angle_to_radian) * vx_set + my_cos((45 + adjusted_angle) * angle_to_radian) * vy_set - wz_set * MOTOR_DISTANCE_TO_CENTER);
    wheel_speed[1] = -(-my_sin((45 + adjusted_angle) * angle_to_radian) * vx_set - my_cos((45 + adjusted_angle) * angle_to_radian) * vy_set - wz_set * MOTOR_DISTANCE_TO_CENTER);
    wheel_speed[2] = -(+my_sin((45 + adjusted_angle) * angle_to_radian) * vx_set - my_cos((45 + adjusted_angle) * angle_to_radian) * vy_set - wz_set * MOTOR_DISTANCE_TO_CENTER);
    wheel_speed[3] = -(+my_sin((45 + adjusted_angle) * angle_to_radian) * vx_set + my_cos((45 + adjusted_angle) * angle_to_radian) * vy_set - wz_set * MOTOR_DISTANCE_TO_CENTER);
}
/**
  * @brief          control loop, according to control set-point, calculate motor current, 
  *                 motor current will be sentto motor
...
static void chassis_control_loop(chassis_move_t *chassis_move_control_loop)
{
    ...
    fp32 angle_motion=chassis_move_control_loop->chassis_yaw_motor->relative_angle ; //获得当前云台与底盘的相对角度rad 
    //mecanum wheel speed calculation
    //麦轮运动分解
    Omnidirectional_wheels(chassis_move_control_loop->vx_set, chassis_move_control_loop->vy_set,
	                       chassis_move_control_loop->wz_set, angle_motion, wheel_speed);
