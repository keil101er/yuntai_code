...
{
   	uint8_t RefereeSendBuf[]={0};                     /* 发送到裁判系统的数据 */
	memset(&RefereeSendBuf,0,sizeof(RefereeSendBuf));//数组内存清0
   unsigned char *framepoint;                      //读写指针
   uint16_t frametail=0xFFFF;                        //CRC16校验值
   
   UI_Packhead framehead;               //7
   UI_Data_Operate datahead;           //6
   UI_Data_Delete del;                //2
   ...
   memcpy(RefereeSendBuf,&framehead,sizeof(framehead));
	 memcpy(RefereeSendBuf+7,&datahead,sizeof(datahead));
	 memcpy(RefereeSendBuf+13,&del,sizeof(del));
	 memcpy(RefereeSendBuf+15,&frametail,sizeof(frametail));
	 Judge_Send_Data(RefereeSendBuf,2*17);
                                                        //包序号+1
}
/************************************************绘制直线*************************************************
...
int UI_ReFresh(int cnt,...)
{
   int i;
	static uint8_t seq;
	uint8_t RefereeSendBuf[15+15*cnt];                     /* 发送到裁判系统的数据 */
	memset(&RefereeSendBuf,0,sizeof(RefereeSendBuf));//数组内存清0
   ...
   va_list ap;                                     
   va_start(ap,cnt);                               //// 初始化 ap 以访问可变参数
   ...
   framehead.Seq=seq;
   framehead.CRC8=Get_CRC8_Check_Sum_UI(framepoint,4,0xFF);
   framehead.CMD_ID=UI_CMD_Robo_Exchange;                   //填充帧头数据
...
//   memcpy(RefereeSendBuf,&framehead,sizeof(framehead));
	     RefereeSendBuf[0] = 0xA5;
    RefereeSendBuf[1] = framehead.Data_Length ;
    RefereeSendBuf[2] = (framehead.Data_Length >> 8);
    RefereeSendBuf[3] = seq;
		
    append_CRC8_check_sum(RefereeSendBuf ,5); //帧头crc8校验
		
    RefereeSendBuf[5] = 0x0301 &255;//命令码
    RefereeSendBuf[6] = (0x0301 >>8);
	 memcpy(RefereeSendBuf+7,&datahead,sizeof(datahead));
   ...
   for(i=0;i<cnt;i++)
   {
      ...
      memcpy(RefereeSendBuf+13+15*i,&imageData,sizeof(imageData));		 //发送图片帧	 
   }
	    memcpy(RefereeSendBuf+13+15*cnt,&frametail,sizeof(frametail));      //发送CRC16校验值  
   Judge_Send_Data(RefereeSendBuf,2*(15+15*cnt));
	 va_end(ap);                                                           //清理ap
   seq++;                                                        //包序号+1
	 if(seq == 1) seq = 0;
   return 0;
...
{
	 uint8_t RefereeSendBuf[60];                  /* 发送到裁判系统的数据 */
   ...
   frametail=Get_CRC16_Check_Sum_UI(framepoint,sizeof(imageData),frametail);             //CRC16校验   //CRC16校验值计算（部分）
    ...
    RefereeSendBuf[5] = 0x0301 &255;//命令码
    RefereeSendBuf[6] = (0x0301 >>8);
//   memcpy(RefereeSendBuf,&framehead,sizeof(framehead));           //发送帧头
	 memcpy(RefereeSendBuf+7,&datahead,sizeof(datahead));            //发送操作数据
	 memcpy(RefereeSendBuf+7+6,&imageData,sizeof(imageData));         //发送图片帧
	 memcpy(RefereeSendBuf+7+6+45,&frametail,sizeof(frametail));        //发送CRC16校验值
   Judge_Send_Data(RefereeSendBuf,7+6+45+2);
   seq++;                                                         //包序号+1
	 if(seq == 1) seq = 0;
   return 0;
}
