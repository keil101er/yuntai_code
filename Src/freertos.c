...
osThreadId user_task_handle; //ui
...
void MX_FREERTOS_Init(void) {
  ...
  /* add threads, ... */
    osThreadDef(cali, calibrate_task, osPriorityNormal, 0, 512);//校准
    calibrate_tast_handle = osThreadCreate(osThread(cali), NULL);

//    osThreadDef(ChassisTask, chassis_task, osPriorityAboveNormal, 0, 512);//底盘
//    chassisTaskHandle = osThreadCreate(osThread(ChassisTask), NULL);

//    osThreadDef(DETECT, detect_task, osPriorityNormal, 0, 256);//裁判系统设备离线
//    detect_handle = osThreadCreate(osThread(DETECT), NULL);

    osThreadDef(gimbalTask, gimbal_task, osPriorityHigh, 0, 1024);//云台
    gimbalTaskHandle = osThreadCreate(osThread(gimbalTask), NULL);

    osThreadDef(imuTask, INS_task, osPriorityRealtime, 0, 1024);//姿态演算
    imuTaskHandle = osThreadCreate(osThread(imuTask), NULL);

    osThreadDef(led, led_RGB_flow_task, osPriorityNormal, 0, 256);//RGB检测
    led_RGB_flow_handle = osThreadCreate(osThread(led), NULL);

//osThreadDef(OLED, oled_task, osPriorityLow, 0, 256);//官方OLED
...
//    osThreadDef(SERVO, servo_task, osPriorityNormal, 0, 128);//舵机任务
//    servo_task_handle = osThreadCreate(osThread(SERVO), NULL);

    osThreadDef(UI , user_task , osPriorityNormal, 0, 128);//UI界面
